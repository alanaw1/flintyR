...
Our first goal is to check whether each cluster of cells looks homogeneous. We run our test on Cluster 14 (which was loaded above), using features grouped into sets based on TAD membership identified by **DomainCaller**.
...
Thus, our test shows evidence of heterogeneity in the clusters and cell type subclusters, after accounting for either (1) correlations induced on peak read counts by the 3D structure of the genome, or (2) chromosome. However, as noted in our paper, an equally valid interpretation is that our accounting for chromatin-induced statistical dependencies between peaks is insufficient.
