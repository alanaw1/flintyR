...
We simulate $\mathbf{X}$, a sample consisting of $N=100$ observations, where each observation is comprised of $P=50$ independent features, each feature a realization of a standard Gaussian random variable. Mathematically, the entries $x_{np}\overset{\text{iid}}{\sim} N(0,1)$. 
...
Under such settings $\mathbf{X}$ is homogeneous and the sample $\mathbf{x}_1,\ldots,\mathbf{x}_N$ is exchangeable, but if we were handed $\mathbf{X}$ without knowing its generative mechanism completely, we might want to perform clustering as part of exploratory data analysis. 
...
By performing $k$-means clustering with $k=2$ as shown above, we obtain a cluster assignment. We can visualize this clustering by performing PCA on the samples and plotting the projection of each observation onto its first two principal components. 
...
![](intro_files/figure-html/2_3-1.png)
...
X_df_1 <- X_df %>% subset(cluster == 1)
X_df_2 <- X_df %>% subset(cluster == 2)
...
![](intro_files/figure-html/2_5-1.png)
...
#> No evidence of heterogeneity at alpha = 0.05!
```
...
To simulate $\mathbf{X}$, we use **msprime** ([this software](https://github.com/tskit-dev/msprime)). The simulation script is saved as `msprime_model.py`, and we call it using the `reticulate` library that interfaces R with Python.
...
# View
X_data$data[1:6,1:6]
#>      rs1_1 rs1_2 rs1_3 rs1_4 rs1_5 rs1_6
#> [1,]     0     0     0     0     0     0
#> [2,]     0     0     0     0     0     0
#> [3,]     0     0     0     0     0     0
#> [4,]     0     0     0     0     0     0
...
As in the basic example, we group the sample into two clusters. Here, we use Prevosti's distance ([Prevosti et al., 1975](https://link.springer.com/content/pdf/10.1007/BF00831894.pdf)), a popular measure of genetic distance, and perform hierarchical clustering with Ward linkage.
...
Similar to Section 2, we can visualize our clusters by plotting the projection of each individual in $\mathbf{X}$ onto its first two PCs. Here, because the number of features is larger than the number of samples, we use Q-PCA instead of R-PCA, and we subsequently obtain the projection scores by right-multiplying $\mathbf{X}$ with the rotation matrix.  
...
![](intro_files/figure-html/3_3-1.png)
...
#> The number of significant variants at FWER = 0.05 is 22.
```
...
![](intro_files/figure-html/3_5-1.png)
...
Like in the basic example, we apply our exchangeability test before performing clustering to avoid double dipping. However, for population-genetic datasets, we expect correlation between SNPs lying in the same chromosome (owing to genetic recombination). This feature-feature correlation induces correlational patterns between individuals, which could falsely suggest the presence of heterogeneous communities. We can see this by running the independent features version of the test.      
